...
<script>
    ...
    // Function to calculate and update total fee
    function updateTotalFee(selectedServices) {
        var totalFee = 0;
        selectedServices.forEach(function(serviceId) {
            var service = serviceList.find(s => s.id == serviceId);
            if (service) {
                totalFee += parseFloat(service.fee);
            }
        });
        $('#total_fee_amount').text(totalFee.toFixed(2));
    }
    
</script>

<!-- Placeholder for total fee calculation -->
